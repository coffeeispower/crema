...
class Panic : Throwable {
    ...
    override fun printStackTrace(s: PrintStream?) {
        ...
        val topOfStack = Arrays.stream(stackTrace)
            .filter { e: StackTraceElement? -> e!!.className.startsWith("online.coffeeispower.crema") }.findFirst().orElseThrow<Panic?>(
                Supplier { Panic("Expected stack trace to have a stack trace element inside crema") })
        s!!;
        s.printf(
